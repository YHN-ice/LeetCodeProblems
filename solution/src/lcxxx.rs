...
impl Solution {
    pub fn minimum_moves(nums: Vec<i32>, k: i32, max_changes: i32) -> i64 {
        (|| {
            ...
            let ones:Vec<i64> = nums.into_iter().enumerate().filter_map(|(idx, elem)| {
                match elem {
            ...
            let presum:Vec<_> = ones.iter().scan(0_i64, |state:&mut i64, v:&i64| {*state += *v; Some(*state)}).collect();
            (0..=(3_i64).min(k).min(l_end)).map_while(|save_for_native|{
                let range_len = k-max_changes.min(k-save_for_native);
                match range_len {
                    0=>Some((k - range_len) * 2),
                    _ =>{
                        (0..l_end).zip(range_len..).map_while(|(l,r)| {
                            if r>l_end{
                                None
                            }
                            else {
                                ...
                                let distance_sum =
                                    presum[r_usize-1_usize] - presum[mid_usize]
                                    - (r-1-mid) * ones[mid_usize]
                                    + (mid-l+1) * ones[mid_usize]
                                    - (presum[mid_usize] - (match l {
                                        0=>0,
                                        _=>presum[l_usize-1]
                                    }));
...
mod test {
    ...
    #[test]
    fn test1(){
        assert_eq!(3, Solution::minimum_moves(vec![1,1,0,0,0,1,1,0,0,1], 3, 1))
    }
